...
BeamformerTester::BeamformerTester()
  ...
  , _stream(0)
{
  // std::cout << "Creating instance of BeamformerTester" << std::endl;
}


BeamformerTester::~BeamformerTester()
{
  // std::cout << "Destroying instance of BeamformerTester" << std::endl;
}
  ...
  const float tol)
{
  ...
  // Check each element of CPU and GPU implementation
	if constexpr (std::is_same<T, float>::value || std::is_same<T, __half>::value)
	{
		float gpu_element;
		float cpu_element;
	...
	else
	{
      ...
      ASSERT_TRUE(abs_diff <= 25)  << "Beamformer: CPU and GPU result is unequal for element " << std::to_string(i) << std::endl
			...
			for(int b = 0; b < _conf.n_beam; b++)
			{
				for(int a = 0; a < _conf.n_antenna; a++)
				{
					for(int f = 0; f < _conf.n_channel; f++)
					{
						...
						if constexpr (std::is_same<T, float2>::value)
							out[out_idx] += tmp.x * tmp.x + tmp.y * tmp.y;
						else if constexpr (std::is_same<T, __half2>::value)
							out[out_idx] = __float2half(__half2float(out[out_idx]) + pow(cuCabsf(tmp), 2));
					}
				}
			}
		}
...
*/
TEST_P(BeamformerTester, BeamformerVoltageHalf){
  std::cout << std::endl
    << "-------------------------------------------------------------" << std::endl
    << " Testing voltage mode with T=__half2 (half precision 2x16bit)" << std::endl
    << "-------------------------------------------------------------" << std::endl << std::endl;
  // __half2 t = {0,0};
  test<__half2,__half2>();
}

TEST_P(BeamformerTester, BeamformerVoltageSingle){
  std::cout << std::endl
    << "-------------------------------------------------------------" << std::endl
...
TEST_P(BeamformerTester, BeamformerPowerHalf){
  std::cout << std::endl
    << "-------------------------------------------------------------" << std::endl
...
TEST_P(BeamformerTester, BeamformerPowerSingle){
  std::cout << std::endl
    << "-------------------------------------------------------------" << std::endl
...
INSTANTIATE_TEST_CASE_P(BeamformerTesterInstantiation, BeamformerTester, ::testing::Values(
  ...
  // samples | channels | antenna | polarisation | beam | beamformer type

  bf_config_t{32, 1, 16, 2, 1, BF_TFAP},
  bf_config_t{2048, 1, 16, 2, 4, BF_TFAP},
  bf_config_t{1024, 64, 32, 2, 32, BF_TFAP},
  bf_config_t{1024, 64, 32, 1, 64, BF_TFAP},
  bf_config_t{1024, 64, 64, 2, 32, BF_TFAP},
	bf_config_t{32, 1, 16, 2, 1, SIMPLE_BF_TAFPT},
	bf_config_t{2048, 1, 16, 2, 4, SIMPLE_BF_TAFPT},
	bf_config_t{1024, 64, 32, 2, 32, SIMPLE_BF_TAFPT},
	bf_config_t{1024, 64, 32, 1, 64, SIMPLE_BF_TAFPT},
	bf_config_t{1024, 64, 64, 2, 32, SIMPLE_BF_TAFPT}
));
