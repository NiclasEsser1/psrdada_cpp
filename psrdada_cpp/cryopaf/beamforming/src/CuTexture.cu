...
template<class T>
CudaTexture<T>::CudaTexture(std::size_t width, std::size_t height, std::size_t depth, int dev_id)
...
template<class T>
CudaTexture<T>::CudaTexture(cudaExtent volume)
	: vol(volume)
{
	x = vol.width;
	y = vol.height;
...
template<class T>
void CudaTexture<T>::set(thrust::device_vector<T> vec)
{
...
template<class T>
void CudaTexture<T>::set(thrust::host_vector<T> vec)
{
	T* p = thrust::raw_pointer_cast(vec.data()); // Pointer to global memory
	pitched_ptr = make_cudaPitchedPtr((void *)p, vol.width*sizeof(T), vol.width, vol.height);
	copy_params.srcPtr = pitched_ptr;
...
template<class T>
void CudaTexture<T>::print_layout()
{
	std::cout << "Texture alignment: (x = " << std::to_string(x) << "; y = " << std::to_string(y) << "; z = " << std::to_string(z) << ")" << std::endl;
}

template<class T>
void CudaTexture<T>::resize(std::size_t width, std::size_t height, std::size_t depth)
{
}

template<class T>
void CudaTexture<T>::free_texture()
{
}
