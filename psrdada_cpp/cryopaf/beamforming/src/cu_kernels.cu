...
void simple_bf_tafpt_stokes_I(const T *idata, U *odata, const T *weights, const bf_config_t *conf)
{
	...
	if(tidx < conf->n_samples && tidy < conf->n_beam && tidz < conf->n_channel)
	{
		for(int i = 0; i < conf->n_antenna; i++)
		{
			...
			{
				tmp = complex_mul(idata[in_offset + i * conf->n_channel * conf->n_pol + k],
					  weights[weight_offset + i * conf->n_channel * conf->n_pol + k]);
				real += tmp.x;
				imag += tmp.y;
			}
			odata[out_offset + tidz] += real*real + imag*imag;
		}
	}
}
...
void simple_bf_tafpt(const T *idata, T *odata, const T *weights, const bf_config_t *conf)
{
	...
	if(tidx < conf->n_samples && tidy < conf->n_beam && tidz < conf->n_channel)
	{
		for(int i = 0; i < conf->n_antenna; i++)
		{
			...
			{
			tmp = complex_mul(
					idata[in_offset + i * conf->n_channel * conf->n_pol + k],
					weights[weight_offset + i * conf->n_channel * conf->n_pol + k]);
				odata[out_offset + tidz * conf->n_pol + k].x += tmp.x;
				odata[out_offset + tidz * conf->n_pol + k].y += tmp.y;
			}
		}
	}
}
